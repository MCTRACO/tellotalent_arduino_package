...
SoftwareSerial::SoftwareSerial(int receivePin, int transmitPin, bool inverse_logic, unsigned int buffSize)
{
  ...
  if (isValidGPIOpin(transmitPin))
  {
    ...
    digitalWrite1(m_txPin, !m_invert);
    m_txValid = true;
  }
}
...
void SoftwareSerial::begin(uint32_t speed)
{
  ...
  m_bitTime = 1000000 / speed;  //debug wsq
  enableRx(true);
}
...
void SoftwareSerial::setTransmitEnablePin(int transmitEnablePin)
{
  if (isValidGPIOpin(transmitEnablePin))
  {
    ...
    digitalWrite1(m_txEnablePin, LOW);
  }
  else
  {
    m_txEnabled = false;
...
size_t SoftwareSerial::write(uint8_t b)
{
  ...
  if (m_txEnabled)
  {
    digitalWrite1(m_txEnablePin, HIGH);
  }
  // create tx interrupts to start bit.
  digitalWrite1(m_txPin, HIGH), digitalWrite1(m_txPin, LOW);

  // WaitBitTime(m_bitTime);
  delayMicroseconds(m_bitTime);
  for (uint8_t i = 0; i != 8; i++)
  {
    digitalWrite1(m_txPin, (b & 1) ? HIGH : LOW);
    // WaitBitTime(m_bitTime);
    delayMicroseconds(m_bitTime);
    b >>= 1;
  }
  // Stop bit
  digitalWrite1(m_txPin, HIGH);
  // WaitBitTime(m_bitTime);
  delayMicroseconds(m_bitTime);
  if (m_txEnabled)
  {
...
void IRAM_ATTR SoftwareSerial::rxRead()
{
  ...
  uint32_t start_time = bit_time / 2 - 2;
  uint32_t end_time = bit_time;
  uint8_t rec = 0;
  ...
  if (0 == digitalRead1(m_rxPin))
  {
    for (uint8_t i = 0; i != 8; i++)
    {
      rec >>= 1;
      // WaitBitTime(bit_time);
      delayMicroseconds(bit_time-1);
      if (digitalRead1(m_rxPin))
      {
        rec |= 0x80;
      }
    }
    ...
    delayMicroseconds(end_time);
    if (1 == digitalRead1(m_rxPin))
    {
